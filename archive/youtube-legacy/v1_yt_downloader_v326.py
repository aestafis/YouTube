...
# BLOCK 8 ── 搜索 / 信息抓取
# ★ v326:
#   - _filter_entries 支持 skip_saved 参数
#   - _fetch_channel 改用 extract_flat:True 正确展开多视频
#   - 频道 URL 自动追加 /videos 后缀
# ══════════════════════════════════════════════════════════════
...
def _channel_url_normalize(url):
    """
    ★ v326: 确保频道 URL 指向视频列表页。
    /@xxx → /@xxx/videos
    /channel/xxx → /channel/xxx/videos
    已有 /videos 或 playlist 则不变
    """
    url = url.rstrip('/')
    if 'playlist?' in url: return url
    if url.endswith('/videos'): return url
    # 去掉已有 tab 后缀再加 /videos
...
def _fetch_channel(url, count, cookie_path, cancel_ev=None):
    """
    ★ v326: 使用 extract_flat:True 完整展开频道视频列表。
    自动 normalize URL → /videos tab。
    """
    url = _channel_url_normalize(url)
    opts = {
        'quiet':True,'no_warnings':True,
        'extract_flat': True,          # ★ v326: True 而不是 'in_playlist'
        'skip_download':True,
        'cookiefile':Cfg.fix(cookie_path),
        'ignoreerrors':True,
...
def _filter_entries(entries, count, mode_cfg,
    ...
    """
    ★ v326: skip_saved=True 时跳过已存视频，返回 (results, skipped_count)
    """
...
def _do_search(url, count, cookie_path, mode_cfg,
    ...
    if len(res)<count*Cfg.SEARCH_FALLBACK_RATIO:
        if cancel_ev and cancel_ev.is_set(): return res, skipped
        entries2=_do_search_raw(url, count*6, cookie_path)
        res, skipped=_filter_entries(entries2, count, mode_cfg, saved_ids, skip_saved)
        # 最多再补一轮（共2轮），防无限循环
        if len(res)<count*Cfg.SEARCH_FALLBACK_RATIO and not (cancel_ev and cancel_ev.is_set()):
            entries3=_do_search_raw(url, count*8, cookie_path)
            res, skipped=_filter_entries(entries3, count, mode_cfg, saved_ids, skip_saved)
    return res, skipped
...
def _do_download(items, cookie_path, save_dir,
                 ...
                 subtitle_on=False, table_mark_cb=None, force_ids=None):
...
# ★ v326:
#   - 左侧 4px 色条方案（不碰 checkbox DOM）
#   - ✓已存 徽章移到播放量列左侧
#   - 渲染时已存视频默认 checkbox=False
#   - 顶部"包含已存"开关，切换已存行勾选状态
#   - DRAG_JS 保持 v325 方案
# ══════════════════════════════════════════════════════════════
_DRAG_JS = """
<script>
(function(){
  if (window._yt_drag_v326) return;
  window._yt_drag_v326 = true;
  ...
  function _applyRange(lo,hi,val){
    _allCbs().forEach(function(c,i){
      ...
      if(c.checked!==nv){c.checked=nv;_fire(c);}
    });
  }
  document.addEventListener('pointerdown',function(e){
    ...
    D.targetVal=!cbs[idx].checked; D.origStates={};
  ...
  document.addEventListener('pointermove',function(e){
    ...
    var idx=_cbIndex(el); if(idx<0||idx===D.curIdx) return;
    D.curIdx=idx;
  ...
  document.addEventListener('pointerup',function(){
    ...
    _applyRange(lo,hi,D.targetVal); D.on=false;
  },true);
  document.addEventListener('pointercancel',function(){
    ...
    cbs.forEach(function(c,i){
      ...
      if(ov!==undefined&&c.checked!==ov){c.checked=ov;_fire(c);}
    });
  },true);
})();
</script>
...
_STATUS_COLORS = {
    ...
    'fail':        '#f44336',   # 红
    'skip':        '#9e9e9e',   # 灰
    'downloading': '#ff9800',   # 橙
}
_STATUS_ICONS = {
    'done':'✅','fail':'❌','skip':'⏭','downloading':'⬇'
}
...
class PreviewTable:
    def __init__(self):
        self._items       = []
        self._boxes       = []
        self._bar_widgets = []   # ★ v326: 左侧色条 W.HTML，不含 checkbox
        self._pending_marks = {}
        self._marks_lock  = threading.Lock()
        self._saved_ids   = set()
        self.container    = W.VBox(layout=W.Layout(width='100%'))
        # "包含已存"开关引用，render 后由 Dashboard 注入
        self._incl_saved_cb = None

    def set_saved_ids(self, ids):
        self._saved_ids = set(ids)

    # ── 色条 HTML（仅 4px 宽竖线 + 悬浮 tip）──────────────────
    @staticmethod
    def _bar_html(row_status=None, reason=''):
        if not row_status:
            return '<div style="width:4px;min-height:54px;'  \
                   'background:transparent"></div>'
        color = _STATUS_COLORS.get(row_status,'transparent')
        icon  = _STATUS_ICONS.get(row_status,'')
        tip   = f' title="{icon} {reason}"' if reason else f' title="{icon}"'
        return (f'<div style="width:4px;min-height:54px;'
                f'background:{color};flex-shrink:0;border-radius:2px 0 0 2px"'
                f'{tip}></div>')

    # ── 行背景色（由状态决定）────────────────────────────────
    @staticmethod
    def _row_bg(row_status, i):
        return {'done':'#e8f5e9','fail':'#fce4ec',
                'skip':'#f5f5f5','downloading':'#fff8e1'}.get(
                row_status, '#fff' if i%2==0 else '#f9f9f9')

    # ── 行容器（带动态背景的外层 Box）────────────────────────
    # 注：行容器背景通过 _bg_widgets 更新，不重建 checkbox
    def _render_rows(self, items):
        ...
        for i, r in enumerate(items):
            ...
            cb = W.Checkbox(
                value=(not is_saved),   # ★ v326: 已存默认不勾选
            ...
            # 色条（初始透明）
            bar_w = W.HTML(
                value=self._bar_html(),
                layout=W.Layout(width='4px', min_width='4px'))
            self._bar_widgets.append(bar_w)
            ...
            # ★ v326: ✓已存 徽章放在播放量列左侧（同一 grid cell 内）
            saved_badge = ''
            if is_saved:
                saved_badge = (
            ...
            vh = (f'{saved_badge}'
                  f'<span style="font-size:12px;color:#444">{views}</span>'
                  if views else
                  f'{saved_badge}'
                  f'<span style="color:#ddd;font-size:11px">-</span>')
            ...
            content = W.HTML(
                value=(
                    ...
                    f'grid-template-columns:24px 1fr 80px 68px 54px;'
                    ...
                    f'<div style="text-align:right;padding-right:6px">{vh}</div>'
    ...
    def render(self, items):
        ...
        # ★ v326: "包含已存"开关
        incl_saved = W.Checkbox(
            value=False, description='包含已存', indent=False,
            layout=W.Layout(width='88px'),
            style={'description_width':'auto'})
        self._incl_saved_cb = incl_saved
        ...
        header = W.HTML(
            ...
            'grid-template-columns:24px 1fr 80px 68px 54px;'
            ...
            '<div style="text-align:right;padding-right:6px">已存 / 播放量</div>'
        ...
        note=W.HTML(
            ...
            f'{"  ·  <span style=color:#2e7d32>✓存 = Drive已有</span>" if has_saved else ""}'
            f'</div>')
    ...
    def apply_pending_marks(self):
        ...
        for vid_id,(st,reason) in marks.items():
            for i,item in enumerate(self._items):
                if item.get('id','')==vid_id and i<len(self._bar_widgets):
                    self._bar_widgets[i].value=self._bar_html(st,reason)
                    break

    def clear(self):
        self._items=[]; self._boxes=[]; self._bar_widgets=[]
        self._saved_ids=set(); self._incl_saved_cb=None
        with self._marks_lock: self._pending_marks.clear()
        self.container.children=()
...
class Dashboard:
    ...
    def _build(self):
        ...
        w_dl        =W.Button(description='开始下载',button_style='success',
                              layout=L(width='88px'))
        # ★ v326: "重新下载"（原强制重下），浅灰
        w_redown    =W.Button(description='↺ 重新下载',
                              layout=L(width='96px'),
                              style={'button_color':'#e0e0e0'},
                              tooltip='忽略已下载记录，对勾选视频重新下载（不影响索引外的文件）')
        ...
        def _reset_btns():
            ...
            w_prev.disabled=False;  w_redown.disabled=False
        ...
        def _on_refresh(_):
            ...
            self._log.write('🔄 自动刷新已重启'); self._flush_queue()
        ...
        w_dl.on_click(lambda _: self._on_download(
            *_params(),w_dl,w_pause,w_resume,w_stop,w_redown,_reset_btns,force=False))
        w_redown.on_click(lambda _: self._on_download(
            *_params(),w_dl,w_pause,w_resume,w_stop,w_redown,_reset_btns,force=True))
        ...
        btn_row=W.HBox(
            [w_prev,w_cancel_prev,w_dl,w_redown,
        ...
        _note=W.HTML(
            ...
            '💡 搜索框：关键词 / 单URL / 多URL（每行或空格） / 频道URL · '
            '左侧色条 = 下载状态 · ✓存 = Drive已有 · 已存默认不勾选</div>')
    ...
    def _on_download(self, query, sort, count, cookie, save,
                     w_dl, w_pause, w_resume, w_stop, w_redown,
        ...
        if force and items:
            ...
            self._log.write(f'↺ 重新下载模式：{len(force_ids)} 个（忽略已下载记录）')

        w_dl.disabled=True; w_dl.description='下载中...'
        w_redown.disabled=True
    ...
    def launch(self):
        ...
        self._log.write(f'v326 就绪 | Drive:{msg}')
        self._log.write(f'FRAGS={Cfg.FRAGS} | chunk={Cfg.HTTP_CHUNK_MB}MB | 刷新=2s')
        self._log.write('多URL/频道URL/取消搜索/已存过滤/左侧色条/重新下载')
