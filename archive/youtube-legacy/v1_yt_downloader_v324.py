...
# ══════════════════════════════════════════════════════════════
# BLOCK 1 ── 配置
# ★ v324: MAX_MB 500→2000; FLUSH_THROTTLE 节流
# ══════════════════════════════════════════════════════════════
class Cfg:
...
# ══════════════════════════════════════════════════════════════
# BLOCK 2 ── 工具函数
# ★ v324: _scan_saved_ids 扫描已保存文件; _embed_thumb; 修复 _rename_with_index
# ══════════════════════════════════════════════════════════════
def _fmt_size(b):
    b = int(b or 0)
...
def _scan_saved_ids(save_dir):
    """
    ★ v324: 扫描 save_dir 下所有子目录，提取文件名中 'id__' 前缀的 video_id。
    用于在预览时标注"已存"状态，即使 state.json 丢失也能识别。
    返回 set of video_id strings。
    """
    ids = set()
    base = Cfg.fix(save_dir)
    if not os.path.isdir(base):
        return ids
    _VIDEO_EXTS_SCAN = {'.mp4','.mkv','.webm','.m4v','.avi','.flv'}
    try:
        for root, dirs, files in os.walk(base):
            # 只扫描 2 层深度，避免 Drive 大量文件拖慢
            depth = root[len(base):].count(os.sep)
            if depth > 2:
                dirs.clear(); continue
            for fn in files:
                ext = os.path.splitext(fn)[1].lower()
                if ext not in _VIDEO_EXTS_SCAN: continue
                # 匹配 id__title.ext 格式（来自 outtmpl %(id)s__%(title)s）
                m = re.match(r'^([A-Za-z0-9_\-]{6,16})__', fn)
                if m:
                    ids.add(m.group(1))
    except Exception:
        pass
    ...
    for idx, (vid, title) in enumerate(vid_order, 1):
        ...
        try:
            for fn in list(os.listdir(session_dir)):
                ...
                if '._emb_' in fn: continue          # ★ 过滤临时文件
...
# ══════════════════════════════════════════════════════════════
# BLOCK 7 ── 搜索
# ★ v324: _fetch_url_info 新增，用于 URL 模式搜索预览
# ══════════════════════════════════════════════════════════════
...
def _fetch_url_info(url, cookie_path):
    """
    ★ v324: 抓取单个 YouTube URL 的元信息，返回 item dict 或 None。
    用于 URL 模式下的搜索预览。
    """
    opts = {
    ...
    try:
        with ThreadPoolExecutor(max_workers=1) as ex:
            ...
            info = ex.submit(_ex).result(timeout=30)
...
# ★ v324:
#   - 去掉 overall_pct
#   - restrictfilenames / windowsfilenames 防止 title 含 / 创建子目录
#   - 强制重下：force_ids set
# ══════════════════════════════════════════════════════════════
...
def _do_download(items, cookie_path, save_dir,
                 ...
                 force_ids=None):
    """
    force_ids: set of video_id — 即使 state.is_done() 也强制重下
    """
    ...
    for idx, item in enumerate(items, 1):
        ...
        opts = {
            ...
            # ★ v324: %(id)s__ 前缀 + windowsfilenames 防止 title 含 / 创建子目录
...
# ★ v324:
#   - 恢复 DRAG_JS，改用 'input' 事件触发 ipywidgets backbone sync
#   - saved_ids 参数：渲染时标注已存视频
#   - force_set：强制重下勾选集合
#   - 已存标签显示在标题与播放量之间
# ══════════════════════════════════════════════════════════════

# ★ v324 DRAG_JS：用 'input' 事件触发 ipywidgets backbone model 同步
_DRAG_JS = """
<script>
(function(){
  if (window._yt_drag_v324) return;
  window._yt_drag_v324 = true;
  ...
  function _fireInput(cb){
    // ipywidgets 监听 'change' + 'input' 两个事件
  ...
  document.addEventListener('pointermove', function(e){
    ...
    allCbs.forEach(function(c,i){
      var nv = (i>=lo && i<=hi) ? D.targetVal : D.origStates[i];
  ...
  document.addEventListener('pointerup', function(){
    ...
    D.on = false;
    // pointerup 时再做一次确认性 fireInput，保证最终状态同步
    var allCbs = _getCbs();
    var lo = Math.min(D.startIdx,D.curIdx), hi = Math.max(D.startIdx,D.curIdx);
    allCbs.forEach(function(c,i){
      var fv = (i>=lo && i<=hi) ? D.targetVal : D.origStates[i];
      if (c.checked !== fv){ c.checked = fv; _fireInput(c); }
    });
  }, true);
  ...
  document.addEventListener('pointercancel', function(){
    ...
    var allCbs = _getCbs();
    allCbs.forEach(function(c,i){
      if (c.checked !== D.origStates[i]){
        c.checked = D.origStates[i]; _fireInput(c);
      }
    });
  }, true);
})();
...
class PreviewTable:
    def __init__(self):
        ...
        self._saved_ids     = set()   # ★ v324: 已保存到 Drive 的 video_id
        self._force_set     = set()   # ★ v324: 用户标记"强制重下"的 video_id
        self.container      = W.VBox(layout=W.Layout(width='100%'))

    def set_saved_ids(self, ids):
        """渲染前由 Dashboard 注入已扫描的 saved_ids"""
    ...
    def _row_html(self, i, r, row_status=None, reason=''):
        ...
        # ★ v324: 已存徽章（标题与播放量之间，显示在频道名右侧）
        saved_badge = ''
        if vid and vid in self._saved_ids and row_status not in ('done','skip'):
            force_tip = '（已勾强制重下）' if vid in self._force_set else '（点右键可强制重下）'
            saved_badge = (
                f'<span style="display:inline-block;background:#e8f5e9;'
                f'color:#2e7d32;font-size:10px;border-radius:3px;'
                f'padding:0 4px;margin-left:4px;border:1px solid #a5d6a7;'
                f'vertical-align:middle" title="已保存到Drive{force_tip}">✓已存</span>')
    ...
    def render(self, items):
        ...
        note = W.HTML(
            ...
            f'{"  ·  <span style=color:#2e7d32>✓已存 = Drive中已有该视频</span>" if self._saved_ids else ""}'
            f'</div>')
    ...
    def clear(self):
        ...
        self._saved_ids = set(); self._force_set = set()
        with self._marks_lock: self._pending_marks.clear()
        self.container.children = ()
...
# ★ v324:
#   - URL 搜索预览：抓取元信息渲染列表
#   - _scan_saved_ids 在预览时注入
#   - _auto_flush 节流 FLUSH_THROTTLE
#   - 强制重下按钮
#   - 去掉 overall_pct
# ══════════════════════════════════════════════════════════════
class Dashboard:
    def __init__(self):
        ...
        self._last_flush_t   = 0.0   # ★ v324: 节流时间戳
    ...
    def _auto_flush(self):
        now = time.time()
        if now - self._last_flush_t < Cfg.FLUSH_THROTTLE:
            return
    ...
    def _build(self):
        ...
        acc_set = W.Accordion(
            children=[W.VBox([
                ...
                W.HBox([w_subtitle, W.HTML('&nbsp;'), w_reset_btn],
        ...
        w_force   = W.Button(description='🔁 强制重下',
                             ...
                             tooltip='对已标注"✓已存"或已下载的视频，忽略历史记录强制重新下载')
        ...
        self._w.update({'prev':w_prev, 'dl':w_dl, 'pause':w_pause,
                        'resume':w_resume, 'stop':w_stop, 'force':w_force})
        ...
        w_dl.on_click(lambda _: self._on_download(
            *_params(), w_dl, w_pause, w_resume, w_stop, w_force,
            _reset_btns, force=False))
        w_force.on_click(lambda _: self._on_download(
            *_params(), w_dl, w_pause, w_resume, w_stop, w_force,
            _reset_btns, force=True))
    ...
    def _on_preview(self, query, sort, count, cookie, save, w_prev):
        ...
        if is_url:
            ...
            self._log.write(f'正在获取视频信息: {_trim(query,60)}')
        else:
            self._status.searching(query)
        ...
        def _search():
            try:
                if is_url:
                    # ★ v324: URL 模式——抓取单视频元信息渲染列表
                    item = _fetch_url_info(query, cookie)
                ...
                # 注入 saved_ids
                saved = _scan_saved_ids(save)
                saved |= self._state._dl
    ...
    def _on_download(self, query, sort, count, cookie, save,
        ...
        # ★ v324: 强制重下时，收集所有勾选视频的 id
        force_ids = set()
        if force and items:
            force_ids = {it.get('id','') for it in items if it.get('id','')}
            self._log.write(f'🔁 强制重下模式：忽略历史记录，重下 {len(force_ids)} 个')
    ...
    def launch(self):
        ...
        self._log.write(f'v324 就绪 | Drive:{msg}')
        self._log.write(f'FRAGS={Cfg.FRAGS} | chunk={Cfg.HTTP_CHUNK_MB}MB | 刷新=2s | 节流={Cfg.FLUSH_THROTTLE}s')
        self._log.write('暂停=视频级 | 字幕默认关 | 封面自动嵌入 | 大小上限=2000MB')
        self._status.idle(); self._flush_queue()
