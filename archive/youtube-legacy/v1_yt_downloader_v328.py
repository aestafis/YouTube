...
# ★ v328: 修复多URL正则——用前瞻截断，无分隔符也能正确切割
def _extract_urls(raw):
    # 在下一个 http:// 或 https:// 出现时截断当前URL
...
# BLOCK 8 ── 搜索 / 信息抓取
# ★ v328:
#   - _fetch_channel: 从父级继承 channel 字段，修复 N/A 问题
#   - skip_saved 在线程外读取，mount 后读索引
#   - _do_search 最多补搜 2 轮
# ══════════════════════════════════════════════════════════════
...
def _fetch_channel(url, count, cookie_path, cancel_ev=None):
    """
    ★ v328:
    - extract_flat=True 完整展开
    - 从父级 info 继承 channel 名，修复 N/A
    - 结果按播放量排序（None 排最后）
    """
    ...
    for e in entries:
        ...
        # ★ v328: channel 优先用条目自有值，fallback 用父级
...
# ★ v328:
#   - HBox 结构: [cb, content_html]，st_w 不出现在 HBox 第一层
#     → 彻底消除 checkbox DOM 顺序与 Python boxes 列表错位
#   - 状态方块嵌入 content HTML 的独立 <span> 中
#     mark() 更新整个 content widget（只改状态那一小段 HTML）
#   - DRAG_JS: 回归 v325 基础逻辑 + pointermove 范围外不操作
#   - 勾选已存 / 取消已存 改为两个独立按钮
# ══════════════════════════════════════════════════════════════

# ★ v328 DRAG_JS: 回归 v325 基础 + 范围外保持当前值
_DRAG_JS = """
<script>
(function(){
  if (window._yt_drag_v328) return;
  window._yt_drag_v328 = true;
  ...
  document.addEventListener('pointermove', function(e){
    ...
    if(!el) return;
    var cb2 = null;
    if(el.type==='checkbox') cb2=el;
    else if(el.closest){
      var w=el.closest('.widget-checkbox');
      if(w) cb2=w.querySelector('input[type=checkbox]');
    }
    if(!cb2) return;
    var cbs = _allCbs();
  ...
  document.addEventListener('pointerup', function(){
    ...
    D.on = false;
    // pointerup 时范围内已是目标值，范围外保持不变，无需额外处理
  }, true);
...
# 状态方块颜色/文字（纯 HTML，嵌入 content widget）
_ST_CFG = {
    'downloading': ('#ff9800', '>'),
    'done':        ('#4caf50', '+'),
    'fail':        ('#f44336', 'x'),
...
def _st_span(st=None, reason=''):
    """生成状态方块 HTML span，嵌入 content HTML 中"""
    if not st:
...
class PreviewTable:
    def __init__(self):
        ...
        self._content_widgets= []       # W.HTML 列表，用于更新状态方块
        self._st_states      = []       # 当前状态 (st, reason) per row
        self._pending        = {}
        self._lock           = threading.Lock()
        self._saved_ids      = set()
    ...
    def _content_html(self, i, r, st=None, reason=''):
        ...
        # grid: 序号 | 标题/频道 | 状态方块 | 播放量 | 发布时间 | 时长
        return (
    ...
    def render(self, items):
        if self._is_downloading: return   # 下载中不重建 boxes
        ...
        for i, r in enumerate(items):
            ...
            # ★ v328: HBox 只有 [cb, content]，不插入任何其他 widget
            # 这保证 _allCbs() 的 DOM 顺序与 self._boxes 严格一致
            cb = W.Checkbox(
                value=(not is_saved),
        ...
        header = W.HTML(
            ...
            '<div style="text-align:center" title="状态">*</div>'
        ...
        wd.on_click(lambda _: [setattr(b,'value',False)
                               for b in self._boxes[wf.value-1:wt.value]])
        range_row = W.HBox(
            [W.HTML('<span style="font-size:11px;color:#888;margin:auto 4px">范围:</span>'),
             wf, wt, ws, wd,
             W.HTML('<span style="font-size:10px;color:#aaa;margin:auto 6px">'
                    '或拖拽复选框</span>')],
        ...
        note = W.HTML(
            ...
            f'{"  [v]=已存(蓝,默认不勾选)" if has_saved else ""}'
            f'  [>]=下载中  [+]=完成  [x]=失败'
            f'</div>')

        ctrl = W.HBox(
            [all_cb,
             W.HTML('<span style="font-size:10px;color:#888;margin:auto 4px">已存:</span>'),
             btn_sel_saved, btn_unsel_saved,
             W.HTML('<span style="flex:1"></span>'),
             header],
            layout=W.Layout(width='100%', align_items='center'))
    ...
    # ── 状态方块更新（线程安全，只更新 content widget）─────────
    ...
    def clear(self):
        ...
        self._content_widgets = []; self._st_states = []
...
# BLOCK 11 ── Dashboard
# ★ v328:
#   - _on_preview: skip_saved 在线程外读取；mount 后读索引
...
class Dashboard:
    ...
    def _build(self):
        ...
        w_reset_btn  = W.Button(description='重置下载记录',
                                ...
                                tooltip='清空历史下载记录（不删除文件），下次会重新下载')
        w_reset_idx  = W.Checkbox(value=False, description='同时清空已存索引',
                                  indent=False, layout=L(width='auto'),
                                  style={'description_width':'auto'},
                                  tooltip='同时删除 .yt_index.json，[v]标记消失')
    ...
    def _on_preview(self, query, sort, count, cookie, save,
        ...
        # ★ v328: 在线程外读取 skip_saved 值（避免闭包延迟）
        ...
        def _search():
            ...
            try:
                ...
                self._log.write(f'已存索引: {len(saved)} 个'
                                f'{"  本次跳过已存" if skip_saved else "  不跳过已存"}')

                if itype == 'channel_multi_warn':
                    channels = idata
                    self._log.write(f'检测到 {len(channels)} 个频道URL，只处理第一个')
                    self._log.write(f'频道: {_trim(channels[0],60)}')
                    actual_url = channels[0]
    ...
    def _on_download(self, query, sort, count, cookie, save,
        ...
        # ★ v328: 先清 _last_results，防止 flush 触发 render 重建 boxes
        self._last_results = None
    ...
    def launch(self):
        ...
        self._log.write(f'v328 就绪 | Drive:{msg}')
        self._log.write(f'FRAGS={Cfg.FRAGS} chunk={Cfg.HTTP_CHUNK_MB}MB '
                        f'大小={"无限" if Cfg.MAX_MB==0 else str(Cfg.MAX_MB)+"MB"}')
        self._log.write('历史记录存于 Drive，重启 Colab 后自动读取')
        self._log.write('索引: ' + Cfg.fix(Cfg.INDEX))
