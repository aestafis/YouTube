...
_SB_TMPL = (
    '<div style="font-size:13px;font-family:-apple-system,BlinkMacSystemFont,'
    '"Segoe UI",Roboto,sans-serif;'
    'background:{bg};color:{fg};padding:7px 12px;'
    'border-radius:8px;border:1px solid #2a2a2e;border-left:3px solid {border};'
    'line-height:1.7;margin:2px 0;'
    'white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'
    '<b>[{icon}]</b>&nbsp;&nbsp;{msg}</div>')

def _sb(style, icon, msg):
    styles = {
        'idle':  ('#18181b','rgba(255,255,255,.72)','#52525b'),
        'info':  ('#18181b','#bfdbfe',_ACCENT),
        'ok':    ('#18181b','#d1fae5','#22c55e'),
        'dl':    ('#18181b','#bfdbfe',_ACCENT),
        'pause': ('#18181b','#fed7aa','#f59e0b'),
        'stop':  ('#18181b','rgba(255,255,255,.72)','#71717a'),
        'err':   ('#18181b','#fecaca','#ef4444'),
    }
...
class StatusBar:
    ...
    def downloading(self,idx,n,title):
        if self._progress_cb:
            try: self._progress_cb(0,idx,n)
            except Exception: pass
        self._push(_sb('dl','>',
            f'下载中 {idx}/{n} | {_trim(title,44)}'))
    def update_progress(self,pct,idx,n,title):
        if self._progress_cb:
            try: self._progress_cb(float(pct),idx,n)
            except Exception: pass
        self._push(_sb('dl','>',
...
<style id="yt-v332-theme">
  ...
  .yt-app .widget-label, .yt-app label {{
    color:rgba(255,255,255,.88) !important;
    font-size:14px !important;
  }}
  .yt-app .widget-text input,
  .yt-app .widget-textarea textarea,
  .yt-app .widget-dropdown select,
  .yt-app .widget-intslider input {{
  ...
  .yt-app .widget-text input:focus,
  .yt-app .widget-textarea textarea:focus,
  .yt-app .widget-dropdown select:focus {{
    box-shadow:0 0 0 2px __ACCENT__55 !important;
  ...
  .yt-btn-primary button:hover,
  .yt-btn-secondary button:hover,
  .yt-btn-danger button:hover {{
...
<script>
(function(){
  if(!document.getElementById('yt-v332-style-fix')){
    var st=document.createElement('style');
    st.id='yt-v332-style-fix';
    st.textContent=
      ...
      'border-radius:10px;background:#18181b;border:1px solid #2a2a2e;'+
      ...
      '.yt-rows-box input[type=checkbox]{cursor:pointer;appearance:none;-webkit-appearance:none;'+
      'width:14px;height:14px;border-radius:4px;border:1px solid #4b5563;background:#0f1115;'+
      'display:inline-block;position:relative;vertical-align:middle;}'+
      '.yt-rows-box input[type=checkbox]:checked{background:__ACCENT__;border-color:__ACCENT__;}'+
      '.yt-rows-box input[type=checkbox]:checked::after{content:"";position:absolute;left:4px;top:1px;'+
      'width:3px;height:7px;border:2px solid #fff;border-top:0;border-left:0;transform:rotate(45deg);}'+
      '.yt-row-line:hover{background:#ffffff08 !important;}';
  ...
  var dragState={
    ...
    ptrId:null,host:null,bound:null
  };

  function _rowCbs(){
    var c=dragState.host||document.querySelector('.yt-rows-box');
    if(!c) return [];
  ...
  function _finishDrag(){
    if(!dragState.on){ dragState.on=false; return; }
    dragState.on=false;
    ...
    try{
      if(dragState.host && dragState.ptrId!==null &&
         dragState.host.hasPointerCapture(dragState.ptrId)){
        dragState.host.releasePointerCapture(dragState.ptrId);
      }
    }catch(e){}
    dragState.ptrId=null;
  ...
  function _onDown(e){
    ...
    dragState.ptrId=e.pointerId;
    try{ dragState.host.setPointerCapture(e.pointerId); }catch(err){}
    cb.checked=dragState.targetVal;
  }
  function _onMove(e){
  ...
  function _bindHost(){
    var host=document.querySelector('.yt-rows-box');
    if(!host || dragState.bound===host) return;
    dragState.host=host; dragState.bound=host;
    host.addEventListener('pointerdown',_onDown,{passive:false});
    host.addEventListener('pointermove',_onMove,{passive:true});
    host.addEventListener('pointerup',_onUp,true);
    host.addEventListener('pointercancel',_onCancel,true);
  ...
  _bindHost();
  setTimeout(_bindHost,180);
  setTimeout(_bindHost,600);
...
def _st_span(st=None,reason=''):
    if not st:
        return '<span style="display:inline-block;width:60px;height:20px"></span>'
    color,icon=_ST_CFG.get(st,('#888','?'))
    tip=(f' title="{escape(str(reason),quote=True)}"'
         if reason else '')
    return (f'<span style="display:inline-block;min-width:60px;height:20px;'
            f'line-height:20px;text-align:center;border-radius:999px;'
            f'border:1px solid {color};color:{color};font-size:11px;'
            f'font-weight:600;cursor:default;padding:0 6px;background:transparent"'
            f'{tip}>{icon}</span>')
...
def _row_html(i, r, st=None, reason=''):
    ...
    thumb=(f'https://i.ytimg.com/vi/{vid}/mqdefault.jpg'
           if _is_valid_video_id(vid) else '')
    thumb_html=(f'<img src="{escape(thumb,quote=True)}" '
                'style="width:40px;height:22px;object-fit:cover;'
                'border-radius:4px;flex:0 0 auto;border:1px solid #2a2a2e"/>'
                if thumb else '')
    ...
    return (
        f'<div class="yt-row-line" style="display:grid;grid-template-columns:{_GRID};'
        f'gap:0 6px;align-items:center;min-height:52px;'
        f'padding:4px 8px;background:{bg};'
        f'font-family:{_UI_FONT_FAMILY};'
        f'border-bottom:1px solid #27272a">'
        f'<div style="text-align:center;color:#6b7280;font-size:11px">{i+1}</div>'
        f'<div style="min-width:0;overflow:hidden;display:flex;gap:8px;align-items:center">'
        f'{thumb_html}'
        f'<div style="min-width:0;overflow:hidden">'
        f'<a href="{url}" target="_blank" '
        f'style="color:{_ACCENT};text-decoration:none;font-size:13px;'
        f'font-weight:500;display:block;white-space:nowrap;'
        f'overflow:hidden;text-overflow:ellipsis" title="{title}">{ts}</a>'
        f'<div style="color:#a1a1aa;font-size:11px;white-space:nowrap;'
        f'overflow:hidden;text-overflow:ellipsis">{ch}</div>'
        f'</div>'
        f'</div>'
        f'<div style="text-align:center">{st_s}</div>'
        f'<div style="text-align:right;padding-right:4px;color:#d4d4d8">{vh}</div>'
        f'<div style="text-align:center;font-size:12px;color:#d4d4d8">{dur}</div>'
        f'</div>')
...
class PreviewTable:
    ...
    def render(self,items):
        ...
        btn_saved=W.Button(
            ...
            style={'font_size':'12px','button_color':_ACCENT},
            tooltip='已存+: 勾选所有已存视频\n已存-: 取消所有已存视频勾选')
        try: btn_saved.add_class('yt-btn-secondary')
        except: pass
        ...
        def _toggle_saved(_):
            ...
            btn_saved.style.button_color='#ef4444' if val else _ACCENT
        btn_saved.on_click(_toggle_saved)

        has_saved=bool(self._saved_ids & {r.get('id','') for r in items})
        note=W.HTML(
            f'<div style="font-size:11px;color:rgba(255,255,255,.58);padding:6px 8px;'
            f'border-bottom:1px solid #2a2a2e;">'
            f'<b style="color:#e4e4e7">{len(items)}</b> 条结果'
        ...
        header=W.HTML(
            ...
            f'gap:0 6px;font-size:11px;color:#9ca3af;'
            f'padding:6px 8px;border-bottom:1px solid #2a2a2e;'
            f'position:sticky;top:0;background:#18181b;z-index:5;">'
        ...
        try: ctrl.add_class('yt-control-row')
        except: pass
...
class Dashboard:
    def __init__(self):
        ...
        self._prog_cur_pct     = 0.0
        self._prog_all_pct     = 0.0
        self._status.set_progress_callback(self._on_progress)

    def _progress_html(self,label,pct,color=_ACCENT):
        p=max(0,min(100,float(pct or 0)))
        return (
            f'<div style="margin:3px 0 2px">'
            f'<div style="display:flex;justify-content:space-between;'
            f'font-size:11px;color:rgba(255,255,255,.70);margin-bottom:4px">'
            f'<span>{escape(label,quote=True)}</span><span>{int(p)}%</span></div>'
            f'<div style="height:8px;border-radius:999px;background:#111319;'
            f'border:1px solid #2a2a2e;overflow:hidden">'
            f'<div style="height:100%;width:{p:.1f}%;background:{color};'
            f'transition:width .15s ease"></div></div></div>')

    def _on_progress(self,pct,idx,n):
        try:
            n=max(1,int(n)); idx=max(1,int(idx))
            cur=max(0,min(100,float(pct or 0)))
            allp=((idx-1)+(cur/100.0))/n*100.0
            self._uiq.put('prog_cur',self._progress_html('当前视频进度',cur))
            self._uiq.put('prog_all',self._progress_html('总进度',allp))
        except Exception:
            pass
    ...
    def _flush_queue(self):
        ...
        if 'prog_cur' in pending and 'prog_cur' in self._w:
            self._w['prog_cur'].value=pending['prog_cur']
        if 'prog_all' in pending and 'prog_all' in self._w:
            self._w['prog_all'].value=pending['prog_all']
    ...
    def _do_reset_dl_btns(self):
        ...
        if 'prog_cur' in self._w:
            self._w['prog_cur'].value=self._progress_html('当前视频进度',0)
        if 'prog_all' in self._w:
            self._w['prog_all'].value=self._progress_html('总进度',0)
    ...
    def _build(self):
        ...
        for mn,mc in MODES.items():
            b=W.Button(description=MODE_LABELS.get(mn,mn),
                       layout=L(width='88px',height='30px'),
                       style={'font_size':'12px',
                               'button_color':'transparent'},
                       tooltip=mc['desc'])
            try: b.add_class('yt-btn-secondary')
            except: pass
            def _om(_,_n=mn,_c=mc):
                ...
                for _b2,_n2 in zip(mode_btns,MODES):
                    _b2.style.button_color=(
                        _ACCENT if _n2==_n else 'transparent')
                self._log.write(
                    f'模式: {MODE_LABELS.get(_n,_n)} — {_c["desc"]}')
                self._flush_queue()
            b.on_click(_om); mode_btns.append(b)
        for _b2,_n2 in zip(mode_btns,MODES):
            _b2.style.button_color=(_ACCENT if _n2==self._mode_name else 'transparent')
        ...
        try: w_query.add_class('yt-input')
        except: pass
        w_sort=W.Dropdown(
        ...
        try: w_count.add_class('yt-input')
        except: pass
        ...
        w_sub_quick=W.Button(
            ...
            style={'button_color':'transparent','font_weight':'600'},
        ...
        try: w_sub_quick.add_class('yt-btn-secondary')
        except: pass
        try: w_rebuild_idx.add_class('yt-btn-secondary')
        except: pass
        try: w_reset_btn.add_class('yt-btn-danger')
        except: pass
        ...
        w_prev=W.Button(description='搜索',
                        ...
                        style={'button_color':_ACCENT,'font_weight':'600'},
                        tooltip='搜索并显示预览')
        ...
        w_clear_l=W.Button(description='清空日志',
                           layout=L(width='88px',height='34px'),
                           style={'button_color':'transparent','font_weight':'600'})
        for _b in (w_prev,w_dl):
            try: _b.add_class('yt-btn-primary')
            except: pass
        for _b in (w_pause,w_resume,w_refresh,w_clear_p,w_clear_l):
            try: _b.add_class('yt-btn-secondary')
            except: pass
        for _b in (w_stop,):
            try: _b.add_class('yt-btn-danger')
            except: pass
        ...
        w_scroll=W.Box(
            ...
            layout=L(width='100%',height='420px',overflow='hidden',
                      border='1px solid #2a2a2e',
                      border_radius='8px',
                      box_shadow='0 10px 24px rgba(0,0,0,0.30)',
                      background='#18181b'))
        ...
        note=W.HTML(
            '<div class="yt-muted-note" style="color:#e5e7eb;'
            'background:#18181b;'
            'border:1px solid #2a2a2e;'
        ...
        prog_cur=W.HTML(self._progress_html('当前视频进度',0))
        prog_all=W.HTML(self._progress_html('总进度',0))
        self._w['prog_cur']=prog_cur
        self._w['prog_all']=prog_all
        mode_row=W.HBox(mode_btns,layout=L(margin='0 0 4px'))
        try: mode_row.add_class('yt-seg-wrap')
        except: pass
        root=W.VBox([
            W.HTML('<div style="font-size:15px;font-weight:600;'
                   'margin:4px 0 6px;color:#f4f4f5">'
                   'YouTube 下载器 '
    ...
    def _on_download(self,query,sort,count,cookie,save,
        ...
        def _run():
            ...
            try:
                ...
                fin_all=min(100.0,((done+fails)/total_cnt)*100.0)
                self._uiq.put('prog_cur',self._progress_html(
                    '当前视频进度',100 if sw!='user_stop' else 0))
                self._uiq.put('prog_all',self._progress_html('总进度',fin_all))
