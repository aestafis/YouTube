...
# ══════════════════════════════════════════════════════════════
# BLOCK 2 ── VideoIndex（延迟加载，__init__ 不碰 Drive）
# ══════════════════════════════════════════════════════════════
class VideoIndex:
    def __init__(self):
        self._path  = None
...
# ══════════════════════════════════════════════════════════════
# BLOCK 3 ── State（延迟加载，__init__ 不碰 Drive）
# ══════════════════════════════════════════════════════════════
class State:
...
# ══════════════════════════════════════════════════════════════
# BLOCK 5 ── SearchEnricher（补全发布时间/频道名）
# 补全结果通过 enqueue_fn 入队，不直接改 widget（线程安全）
# ══════════════════════════════════════════════════════════════
...
class SearchEnricher:
    ...
    def start(self, items, cookie_path, enqueue_fn, on_done, enabled=True):
        self.cancel()
            ...
            def _enrich_one(i, item):
                with sem:
                    ...
                    if meta:
                        ...
                        with count_lock:
                            count[0] += 1
                        try: enqueue_fn(i, updated)
                        except Exception: pass
                    time.sleep(0.3)

            for i, item in enumerate(items_snapshot):
                if cancel_ev.is_set(): break
                t = threading.Thread(target=_enrich_one,
                                     args=(i, item), daemon=True)
...
def _do_search(url, count, cookie_path, mode_cfg,
    ...
    res, skipped = _filter_entries(
        entries, count, mode_cfg, saved_ids, skip_saved)
    rounds = 0
    ...
    for idx, item in enumerate(items, 1):
        ...
        if dl_ok:
            ...
            if not tmp_files:
                ...
                main_ok = copy_ok and any(
                    sz>100*1024 for _,_,sz in vid_files)
...
# ══════════════════════════════════════════════════════════════
# BLOCK 11 ── PreviewTable
# 修复: _selected_set(P0-1) / .yt-rows-box隔离(P0-2) /
#       unobserve_all(幽灵回调) / 灰色方块(P0-4) /
#       enqueue_enrichment队列(线程安全)
# ══════════════════════════════════════════════════════════════
_DRAG_JS = """
<script>
(function(){
...
class PreviewTable:
    def __init__(self):
        ...
        # ★ 预览行专用容器，JS 通过 .yt-rows-box 精确定位
    ...
    def render(self, items):
        ...
        # ★ 解绑旧 observe，防止幽灵回调写入 _selected_set
        for old_cb in self._boxes:
            try: old_cb.unobserve_all()
            except Exception: pass
        self._items=[]; self._boxes=[]; self._content_widgets=[]
        self._st_states=[]
        ...
        def _sel_s(_):
            for r,b in zip(self._items,self._boxes):
                if r.get('id','') in self._saved_ids: b.value=True
        def _unsel_s(_):
            for r,b in zip(self._items,self._boxes):
                if r.get('id','') in self._saved_ids: b.value=False
        btn_sel.on_click(_sel_s)
        btn_unsel.on_click(_unsel_s)
...
class Dashboard:
    def __init__(self):
        ...
        self._w                = {}
        self._acc_mod          = None   # 固定模块 Accordion 引用
        self._run_id           = 0
        self._cur_idx          = 0
        self._cur_total        = 0
    ...
    def _build(self):
        ...
        w_sort  = W.Dropdown(
            ...
            description='排序:',
            style={'description_width':'55px'},
            layout=L(width='165px'),
            tooltip='相关性=YouTube算法  最多播放=按播放量  最新上传=按时间')
        w_count = W.IntSlider(
        ...
        for cat, kws in KEYWORD_MODULES.items():
            ...
            for lbl, kw in kws.items():
                ...
                def _ok(_b, _kw=kw):
                    ...
                    ky = _kw.replace('2026',str(yr)).replace(
                        '2025',str(yr-1))
                    w_query.value = ky
                    # ★ 点击后收起 Accordion
                    if self._acc_mod:
        ...
        acc_mod = W.Accordion(
            children=[W.VBox([
                W.HTML('<div style="font-size:10px;color:#888;'
                       'padding:2px 0 4px;border-bottom:1px solid #eee;'
                       'margin-bottom:4px">'
                       '点击填入搜索词 · 悬停查看原词 · 年份自动替换</div>'),
                W.VBox(mod_rows)
        ...
        w_cookie = W.Text(
            ...
            style={'description_width':'55px'}, layout=L(width='97%'),
            tooltip='YouTube Cookie 文件路径，需手动上传到 Drive')
        w_save = W.Text(
            value=Cfg.SAVE_DIR, description='保存:',
            style={'description_width':'55px'}, layout=L(width='97%'),
        ...
        w_maxmb_label = W.HTML(
            value='<span style="font-size:11px;color:#2e7d32;'
                  'margin-left:6px">不限制</span>')
        def _update_label(c):
            v = c['new']
            if v == 0:
                w_maxmb_label.value=(
                ...
                w_maxmb_label.value=(
                    ...
                    f'margin-left:6px">最大 {v} MB</span>')
        ...
        w_skip_saved = W.Checkbox(
            ...
            tooltip='关键词搜索时过滤已下载过的视频（推荐开启）')
        w_enrich = W.Checkbox(
            value=True, description='补全发布时间', indent=False,
            layout=L(width='auto'),
            style={'description_width':'auto'},
            tooltip='搜索完成后异步补全发布时间和频道名（约5-15秒）')
        w_reset_btn = W.Button(
            description='重置记录',
            button_style='warning', layout=L(width='90px'),
            tooltip='清空历史下载记录（不删除实际文件）')
        w_reset_idx = W.Checkbox(
            value=False, description='同时清空已存索引',
            indent=False, layout=L(width='auto'),
            style={'description_width':'auto'},
            tooltip='同时删除 .yt_index.json，蓝色[v]标记消失')
        ...
        self._w.update({'cookie':w_cookie,'save':w_save,
                        ...
                        'skip_saved':w_skip_saved,'enrich':w_enrich})

        # 按钮
        w_prev = W.Button(
            description='搜索预览', button_style='info',
            layout=L(width='88px'), tooltip='搜索并显示预览列表')
        w_cancel_prev = W.Button(
            description='取消', button_style='warning',
            layout=L(width='60px'), disabled=True,
            tooltip='取消当前搜索')
        w_dl = W.Button(
            description='开始下载', button_style='success',
            layout=L(width='88px'), tooltip='下载勾选的视频')
        w_pause = W.Button(
            description='暂停', layout=L(width='72px'), disabled=True,
            style={'button_color':'#f57f17'},
            tooltip='当前视频下载完成后暂停')
        w_resume = W.Button(
            description='继续', button_style='success',
            layout=L(width='72px'), disabled=True,
            tooltip='继续下载下一个视频')
        w_stop = W.Button(
            description='终止', button_style='danger',
            layout=L(width='72px'), disabled=True,
            tooltip='立即终止当前下载')
        # ★ P2-1: 重启刷新，语义明确
        w_refresh = W.Button(
            description='重启刷新', layout=L(width='88px'),
            style={'button_color':'#607d8b'},
            tooltip='状态长时间不更新时点击，重新注入2s自动刷新定时器')
        w_clear_p = W.Button(
            description='清空预览', layout=L(width='80px'),
            style={'button_color':'#607d8b'})
        w_clear_l = W.Button(
            description='清空日志', layout=L(width='80px'),
            style={'button_color':'#607d8b'})
        ...
        w_prev.on_click(
            lambda _: self._on_preview(*_params(), w_prev, w_cancel_prev))
        w_cancel_prev.on_click(
            lambda _: self._on_cancel_preview(w_cancel_prev))
        w_dl.on_click(
        ...
        log_acc = W.Accordion(
            ...
            layout=L(width='100%', margin='2px 0'))
        try:    log_acc.titles = ('下载日志',)
        except: log_acc.set_title(0, '下载日志')
            ...
            try:
                ...
                if itype == 'channel_multi_warn':
                    ...
                    def _prog(d, t):
                        ...
                        self._auto_flush()
                    res  = _fetch_multi_urls(
                        idata, cookie, cancel_ev, _prog)
                    mode = f'{len(res)}个URL'
                ...
                if res:
                    ...
                    if enrich_on:
                        # ★ put_cb 保证在 render() 之后才启动补全
                        def _start_enrich():
                            if not self._table._items: return
                            self._enricher.start(
                                ...
                                on_done     = lambda n: (
                                    ...
                                    if n > 0 else None),
                                enabled     = True)
                        self._uiq.put_cb(_start_enrich)
                else:
                    self._log.write(
                        '未找到符合条件的视频'
    ...
    def _on_download(self, query, sort, count, cookie, save,
        ...
        subtitle_on  = self._w['subtitle'].value   # 线程外读取
        ...
        if itype in ('single_url','multi_url','channel','channel_multi_warn'):
            ...
            if selected is None:
                ...
                items = list(selected)   # ★ P1-3: 不 sorted
        ...
        def _run():
            ...
            try:
                ...
                # ★ _guarded_reset 通过 put_cb 在主线程执行
    ...
    def launch(self):
        ...
        self._log.write('v330 就绪  Drive 将在搜索/下载时自动挂载')
...
try:
    ...
    try: _INSTANCE._enricher.cancel()
    except: pass
except Exception:
    pass
